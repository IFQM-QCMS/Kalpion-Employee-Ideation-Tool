...
test('notifications: the unread badge counts the account, not the visible page', async () => {
  const rows = await sql('ifqm_test_a', "SELECT id FROM ifqm_test_a.users WHERE email='user@orga.test'");
  const uid = rows[0].id;
  // More than one page's worth, so a count taken from the returned rows would under-report -
  // which is exactly what the badge used to do.
  ...
  // Marking ONE read must mark exactly one.
...
test('the single-idea Closure Summary PDF is open to everyone and stops at the tenant boundary', async () => {
  ...
  // Everybody can export. What differs is the contents, not the permission: the PDF is built
  // from ideaService.get(), which has already decided what this particular reader may see.
...
test('a colleague outside an idea gets a summary sheet, not the full record', async () => {
  // The two documents are checked by what actually lands in the bytes, not by what the
  // screen renders.
  ...
  // The shared helper reads bodies with res.text(), which decodes as UTF-8 and mangles a
  // PDF. Take the bytes.
...
test('billing: a plan is priced correctly, and a lapsed organisation is held', async () => {
  // Money is stored in paise. ₹2,500 with GST included must break down to a base and a tax
  // that add back to exactly ₹2,500 - the whole reason prices are not kept as decimals.
  ...
  // A paid plan cannot carry a trial period any more: an evaluation runs on the Trial plan,
  // and a paid plan starts paying.
  ...
  // Past the grace window, it really is over. Expiry is derived from the dates on every
  // read, not from a flag some sweep has to set first.
  await sql('ifqm_test_master',
    `UPDATE ifqm_test_master.tenants
  ...
  // With enforcement off - the default - the sweep reports and changes nothing anybody would
  // notice.
...
test('billing: an organisation on hold can still sign in and pay - and do nothing else', async () => {
  // The failure this guards against: an organisation put on hold for non-payment could not
  // sign in at all (tenant resolution matched status='active' only), and even before the
  // hold, the billing gate refused /settings/billing along with everything else.
  const [orgb] = await sql('ifqm_test_master', "SELECT id FROM ifqm_test_master.tenants WHERE slug='orgb'");
  const plans = (await api('GET', '/api/platform/plans', { token: PA })).data.plans;
  const starter = plans.find((p) => p.code === 'STARTER');

  // On a plan, because recording a payment for an organisation nobody has priced is refused
  // - and this test ends by recording one.
  ...
  // Paying is reached on its own terms - 503 here is "no gateway configured", which is a
  // different answer from "your access is paused".
  ...
  // Opening the page to everyone signed in is deliberate - "we are days from being cut off"
  // is not confidential from the people it will cut off - but spending the company's money
  // is not.
  ...
  // A suspension an operator applied by hand is a different thing, and stays a hard refusal
  // - otherwise every organisation that had ever lapsed would keep the softer treatment
  // forever, whatever it was later suspended for.
...
test('billing: the grace window the API enforces is the configured one', async () => {
  // The sweep, the overview and the organisation's own billing page all passed the
  // configured window to billingState(); the per-request gate did not, so it used the
  // built-in two days regardless.
  const [orgb] = await sql('ifqm_test_master', "SELECT id FROM ifqm_test_master.tenants WHERE slug='orgb'");
...
test('the request allowance comes from the plan, and can never lock a workspace out', async () => {
  // A flat 2,000-a-month cap once took a live customer offline: it was an integration
  // allowance applied to ordinary page loads.
  let res = await api('GET', '/api/platform/plans', { token: PA });
  const starter = res.data.plans.find((p) => p.code === 'STARTER');
  ...
  const setUsage = async (n) => {
    ...
    // Assigning the plan again drops the cached allowance, so the next request re-reads it.
    await api('POST', `/api/platform/tenants/${orgb.id}/plan`, {
      token: PA, body: { plan_id: planId, trial_days: 0 },
    });
  };

  // Just over the line is tolerated: the allowance estimates normal use, it does not measure
  // it, and somebody 5% over is busy rather than abusive.
  await setUsage(1050);
  ...
  // ...but the allowlist still answers. A customer who has run out has to be able to sign
  // in, see why, and raise a ticket about it.
...
test('a user signs in with just their email - no organisation code needed', async () => {
  // Note the missing third argument: org_slug is empty, so the platform must work out the
  // tenant from the email itself (login directory tenant scan).
...
test('QCMS integration: approved-ideas list, key masking, and the push flow', async () => {
  ...
  // Exercise the default (no per-tenant override) path: point the runtime config at the mock
...
test('a QCMS duplicate-key leak (HTTP 500) is treated as a duplicate, not a failure', async () => {
  // Some QCMS builds return a raw Postgres unique-constraint error with a 500 instead of the
  // documented 409 when an idea already exists.
...
// These exist because the one-time-code feature shipped complete and unreachable: `otp_*`
// was seeded into platform_settings and appeared on no whitelist, so nothing in the
// product could ever set otp_enabled to 1.
test('messaging: the gateway key and the mail token are never returned, and an empty field keeps them', async () => {
  ...
  // The hazard this guards: saving an unrelated field used to blank the credential, because
  // an untouched password input posts an empty string.
...
test('messaging: code sign-in cannot be switched on while the gateway could not deliver', async () => {
  ...
  // Connector off - enabling must be refused rather than putting a sign-in method on the
  // login screen that silently never works.
  ...
  // A six-character header is the DLT rule; five is a transcription slip the gateway would
  ...
  // Everything present - now it may be enabled.
...
test('messaging: the DLT template is filled from the registered wording, not from a literal', async () => {
  ...
  // The check that matters: a message built from the registered template matches it, and one
  // whose wording has drifted does not.
...
// The whole platform on hold while developers work on an update.
test('maintenance: tenants are locked out, IFQM staff are not, and it reverses cleanly', async () => {
  ...
  // ...nor by one-time code, which is the same door and would otherwise be left wide open.
  ...
  // ...but that user can still log out, rather than being stuck holding a token every
  // endpoint refuses.
  const out = await api('POST', '/api/auth/logout', { token: before.token });
  assert.equal(out.status, 200);

  // 4. IFQM staff are unaffected - they can still sign in and still reach the console, which
  ...
  // 7. And with maintenance OFF, the switch is still staff-only - the ordinary authorisation
  // check, no longer masked by the 503 above.
...
// Two rules, both asked for after organisations turned up priced at Rs.50,000 while
// showing "Trial - 14 days left": a trial runs on the Trial plan and nothing else, and the
// Trial plan itself cannot be deleted because every newly approved organisation is put on
// it.
test('billing: a trial runs on the Trial plan, and that plan cannot be deleted', async () => {
  ...
  // And the same paid plan is fine the moment the trial length is zero, which is what
  // converting a customer looks like.
...
test('profile: a user may fix their own details but cannot promote themselves', async () => {
  ...
  // Points are earned by earlier cases in this file, so the property under test is that they
  // are UNCHANGED by this call - not that they are zero.
...
test('branding: an uploaded logo survives the uploads folder being wiped', async () => {
  ...
  // An employee must see it at all - this is the sidebar for everybody, not just for whoever
  // uploaded it.
...
// One approval chain
// The chain used to be described four ways at once: a built-in role pair, an ordered stage
// list, a free-form pair of role checkbox sets, and a committee percentage that overrode
// the lot.
...
test('the approval chain is derived from the stage list, and the dead keys are refused', async () => {
  ...
  // The four removed keys must not be writable.
  ...
  // A chain with no approver in it is refused rather than stored - it would leave every
  // submitted idea with nobody able to action it.
  ...
  // An unknown stage key is dropped, not stored: a step nobody holds is a step no idea can
  // pass, and it would only surface as a stuck submission.
  await api('POST', '/api/settings', {
    token: AADMIN,
...
test('a review committee decides unanimously - one rejection is enough', async () => {
  // Two real reviewers, so "all of them" is a meaningful claim rather than an accident of
  ...
  const mk = async (email, name, phone) => {
    const created = await api('POST', '/api/users', {
      ...
      body: {
        ...
        // Every account needs an employee ID and a mobile number, whatever creates it - see
        // createUser.
  ...
  // Org admins hold no approval authority (§13.12) and may not route either, so the routing
  // is done by a manager - which is who does it in practice.
  ...
  let res = await api('POST', '/api/ideas/assign-reviewers', {
    token: router,
    // The old threshold field, still sent by a stale client.
    body: {
...
// An approved idea and one that has been handed to the QC tool as tracked work looked
// identical everywhere outside the org admin's own Approved Ideas tab.
test('an idea forwarded to QCMS is distinguishable from one merely approved', async () => {
  ...
  // Now forwarded. The screens read qcms_push_status, so it has to survive the trip on every
  // payload that shows a status badge.
  await sql('ifqm_test_a',
  ...
  // The board names its columns instead of selecting i.*, so it was the one screen where the
  // mark would silently never appear.
...
// users.email was NOT NULL UNIQUE and was the login identifier, so an employee with no
// company mailbox - most of a shop floor - had to be given a fabricated address before an
// account could exist, which then received nothing and was still what they had to type to
// sign in.
...
test('a user can be created with a username and no email, and sign in with it', async () => {
  ...
  // Case is not part of the identity - somebody typing it from a handset gets a capital
  // first letter whether they meant one or not.
  const upper = await api('POST', '/api/auth/login', {
    body: { email: 'Yashas123', password: 'UsernamePass123' },
  });
  assert.equal(upper.status, 200, 'a username must not be case-sensitive');

  // A second account with no email must not collide with the first on the UNIQUE index -
  // NULLs do not collide, empty strings would have.
...
test('an account must have a username or an email, and a username is platform-wide', async () => {
  ...
  // Malformed usernames are refused rather than stored - each of these would land in the
  // same keyspace as an email or a phone key and could shadow somebody else's sign-in.
  ...
  // The one that matters: a username is unique across the WHOLE platform, not per
  // organisation, because login_directory is keyed on the identifier alone - that single key
  // is what lets somebody sign in without an org code.
...
// Lifetime plan
// billing_cycle already had 'one_time', mapped to 3650 days and commented "effectively
// perpetual; still has an end date on file".
test('a lifetime plan never expires and is never billed again', async () => {
  ...
  // The label has to separate "sold a perpetual plan" from "we chose not to bill them" -
  // both are exempt, and the screens should not say the same thing.
  ...
  // The two calls that would quietly undo it.
...
// MOM 29 Jul 2026 §13. The statutory step was removed wholesale for a while - Udyam,
// GSTIN, PAN, CIN and the website all came off the form - and §13 puts back exactly two of
// them: GSTIN and Company PAN.
test('an application needs its business identity, and nothing §13 calls optional', async () => {
  ...
  // The "other details". Absent, an application still goes through - this is the half of §13
  // that is easy to overlook, because the fields were all mandatory before and nothing
  // complains when they simply stay filled in.
  ...
  // A CIN or a Udyam number is not asked for, and is still stored when sent - an older
  // client, or a later step, must not silently lose what it supplied.
...
// registrationService has carried a FREE_EMAIL_DOMAINS list since it was written, with a
// comment explaining why an application from Gmail is either a sole trader on personal
// email or noise - and nothing ever consulted it.
test('applications from personal mailboxes are refused unless allowed explicitly', async () => {
  ...
  // Disposable is refused for a different reason, and is not whitelistable at all - an
  // address designed to stop existing is not a small business without a domain.
  ...
  // Everybody else on that provider is still refused - this is the whole point of allowing
  // an address rather than a domain.
  ...
  // The live check the signup form calls as you type must agree with the rule the submit
  // path enforces, or the form says yes and the server says no.
  ...
  // MOM 24/08: no differentiation by organisation size or type - any organisation may be
  // admitted through the whitelist, subject to IFQM approval.
...
// The largest attachment any organisation could allow was MAX_FILE_MB, an environment
// variable, so raising it for one customer meant a redeploy.
test('an organisation may lower the attachment limit but never raise it past the platform ceiling', async () => {
  ...
  // The org admin sees the bound their field is actually clamped to. Without this the form
  // advertises one number while the server enforces another.
  ...
  // Higher is not. Trimmed to the ceiling rather than refused, so an admin who types an
  // optimistic number still ends up with a working setting.
  ...
  // Lowering the ceiling afterwards must bind an already-stored value too.
  await setCeiling(5);
  seen = await api('GET', '/api/settings', { token: AADMIN });
...
// An application landed in a queue nobody was watching.
test('a new application notifies every platform admin, and cannot fail the submission', async () => {
  ...
  // Nothing actually leaves the machine in a test run - platform mail is blanked by the
  // harness - so `sent` is expected to be 0 here.
...
// Approving a registration and resetting an org admin's password both minted a credential
// and put it on screen with "share it with the applicant".
test('a temporary password is emailed, and is still shown when the email cannot go', async () => {
  ...
  // Platform mail is blanked by the harness, so this exercises the path that matters most:
  // what happens when the send does NOT work.
  ...
  // No address is a normal state, not an error - the same rule the mailer applies everywhere
  // since accounts stopped requiring an email.
  ...
  // The console reset still hands the password back whether or not the email went.
  ...
  // Sent as a boolean or a 1 depending on the driver's tinyint handling - the fact being
  // asserted is that it is set, not how MySQL spelled it.
  assert.ok(signedIn.data.user.must_change_password,
    'and must force a change at first sign-in');
});

// Archiving a ticket
// The bulk endpoint has only ever archived resolved and closed tickets.
test('only a resolved ticket can be archived, one at a time or in bulk', async () => {
  ...
  // Restoring is never restricted - getting something back must be easier than losing it.
...
// There are three manuals and handing an employee the platform-admin one is not a small
// mistake: it describes a console they have no account for and other organisations they
// must never learn exist.
test('each role is served its own manual, and none is served anonymously', async () => {
  ...
  // Filename is how the caller can tell WHICH manual arrived: the body is a PDF either way,
  // and asserting on bytes would prove nothing about routing.
  ...
  // Skipped rather than failed on a deployment shipped without the folder - that is a
  // packaging choice, and the 404 path is asserted above.
...
test('the closure PDF grows with the approval chain it has to show', async () => {
  ...
  // Six steps, each with a comment long enough to wrap - which is also what exercises the
  // row-height and page-break arithmetic.
  for (let i = 1; i <= 6; i++) {
    await sql('ifqm_test_a',
...
// The code route existed on the server and nothing ever called it: the forgot dialog
// validated an email regex and refused everything else.
test('a reset code can be requested by username or phone, and goes somewhere reachable', async () => {
  ...
  // classify() has to know the third identifier, or the reset path rejects it before it ever
  // looks anybody up.
  ...
  // The code must be redeemable against the DESTINATION, not the username - keying the row
  // on what was typed would issue a code nobody could use.
  const [row] = await sql('ifqm_test_master',
  ...
  // An unknown identifier is answered identically - this must not become a way to discover
  ...
  // Nonsense is refused outright rather than answered generically: there is nothing to
  // protect, and the person has simply mistyped.
  await assert.rejects(() => auth.requestPasswordResetCode({ identifier: '  ' }),
    /username, registered email address or mobile number/i);
});

// The code has to be redeemable end to end, or the dialog is telling somebody to type it
// into a box that leads nowhere.
test('a code sent to a username holder resets the password end to end', async () => {
  const auth = await import('../src/services/authService.js');

  // The previous case already asked for a code for this account, and the resend throttle is
  // real - sixty seconds.
  await sql('ifqm_test_master',
  ...
  // And the whole point: the person can now get in, by the username they started from.
  const signedIn = await api('POST', '/api/auth/login', {
...
// Log retention
// "Delete the audit logs after two or three years" reads as one instruction and is really
// two.
test('the purge deletes old access logs and never touches approval or billing history', async () => {
  ...
  // Old and recent sign-in records, aged by the DATABASE's clock - the same clock the purge
  // compares against.
  await sql('ifqm_test_master',
  ...
  // A dry run counts without deleting, so a change to the window can be sized before it is
  // made.
  ...
  // The half that must never happen. idea_workflow is the approval chain - it is Section H
  // of the closure PDF - and it is not in the purge list at all.
  ...
  // The window is a setting, and is floored so it cannot be set somewhere that would take
  // this quarter's lockout counters with it.
  await api('PUT', '/api/platform/settings/defaults', {
...
// Implementation Rate reported counts['Implemented'] under the label "Ideas forwarded to
// QC".
test('analytics reports what reached QC, separately from what is marked Implemented', async () => {
  ...
  // Earlier cases in this file have already forwarded ideas, so both figures are asserted as
  // DELTAS.
  const baseVelocity = Number(before.data.qcms.pushed_30d) || 0;

  // An idea marked Implemented but never pushed. Under the old metric this alone moved
  // Implementation Rate; it must now move nothing.
  const submit = await api('POST', '/api/ideas/submit', {
    token: AUSER,
    body: {
  ...
  // Velocity is a pace, so it only counts the recent window.
  await sql('ifqm_test_a',
...
// Pay as you go
// Billed for the people who actually signed in, not for the accounts that exist.
test('pay as you go meters distinct sign-ins, and a closed month stops moving', async () => {
  ...
  // Three sign-ins from two people, in this month. One active user signing in repeatedly is
  // one active user - that is the whole point of DISTINCT.
  await sql('ifqm_test_master',
    `INSERT INTO ifqm_test_master.platform_login_activity
  ...
  // The guarantee the snapshot exists for.
  await sql('ifqm_test_master',
    `INSERT INTO ifqm_test_master.platform_login_activity
       (actor_type, actor_id, actor_name, tenant_id, outcome, created_at)
  ...
  // Renewal by fixed period is refused: a PAYG month is settled against what it metered, and
  // extending it would move the period end with nobody having decided what was owed.
...
// Pools were created lazily and never evicted, so open connections grew monotonically with
// the number of distinct organisations touched since the last restart.
test('the tenant pool cache is capped and evicts the least recently used', async () => {
  ...
  // Ask for more distinct pools than the cap allows.
  ...
  // The point of LRU rather than "evict anything": a tenant asked for repeatedly must
  // survive a flood of one-off tenants.
...
// An organisation set its chain to start at Manager, and its team leads went on approving
// ideas with full authority.
test('a role outside the configured approval chain cannot approve, and sees an empty queue', async () => {
  ...
  // And the queue. A bare `else` used to send every out-of-chain role to the org-wide
  // branch, so an excluded team lead saw EVERY idea in the organisation - more than they
  // should see, and how they reached them.
...
test('the import template no longer has a birth column', async () => {
  ...
  // An.xlsx is a zip, so its strings are compressed - searching the bytes proves nothing
  // either way. Open it properly and read the header row.
...
test('the leaderboard PDF renders names in every language the product ships in', async () => {
  ...
  // Noto Sans has no Kannada, Tamil, Telugu or Malayalam glyphs, and a missing glyph in that
  ...
  // And the Indic faces must actually be embedded - a notdef count of zero would also be
...
test('one long name cannot break the leaderboard PDF layout', async () => {
  ...
  // PDFKit's { ellipsis: true, lineBreak: false } did not hold: an over-long name wrapped
  // onto a second line, and because the row separators are drawn at a fixed height the
  // overflow spilled through the rule and out of the table.
...
test('an unshapeable string degrades instead of taking the export down', async () => {
  ...
  // fontkit crashes shaping "ప్ర" in Noto Sans Telugu.
  const rows = [
...
test('every OTP journey sends exactly the wording its template was approved under', async () => {
  ...
  for (const [purpose, [id, body]] of Object.entries(APPROVED)) {
    ...
    // One segment. These are transactional codes; a body that runs past 160 characters is
    // billed and delivered as two, and a concatenated OTP is exactly the kind of message that
    // arrives out of order or half-missing.
    assert.ok(m.text.length <= 160,
      `${purpose} is ${m.text.length} characters - over one SMS segment`);
  }

  // The template must carry exactly one variable: the approved wording has no expiry in it,
  // and a second {#var#} would leave a literal in the message.
...
test('a template with no registration and no fallback is refused, not borrowed', async () => {
  ...
  // 'kaleyra' rather than the default, because the log provider is the local mock and is
  // deliberately exempt from this refusal.
  ...
  // Either wording is right, and which one appears says which branch caught it: a purpose
  // the file has never heard of reports "not configured", a known one still awaiting its id
...
test('the registered sender header is accepted', async () => {
  ...
  // IFQMID-T is a six-character DLT header plus the transactional category suffix.
  // Six characters go on the wire.
...
test('a template awaiting its id borrows a whole registration, never half of one', async () => {
  ...
  // Verifying a NEW number on an existing account has its own wording submitted but no id
  // yet.
  ...
  // And the wording it will use once approved is ready and distinct - if these were the same
  // string, the fallback would be pointless and somebody would eventually delete it as dead
  // code.
...
test('granting the pending id switches a template to its own wording', async () => {
  ...
  // The documented next step is "paste the id in, set registered true". This asserts that
  // really is the whole change.
...
test('an idea travels every stage of the chain and is Approved only at the last', async () => {
  ...
  // Out of turn is refused. The plant head is the FINAL approver and could, under the old
  // engine, simply approve - closing the idea past three stages whose holders never saw it.
  res = await api('POST', '/api/ideas/review-action', {
...
test('an idea is only pushable to QCMS after the final stage', async () => {
  // The QCMS list is gated on status='Approved'.
  await api('POST', '/api/settings', {
    token: AADMIN,
...
test('bulk approve obeys the chain instead of writing the status directly', async () => {
  // bulkReview used to run `UPDATE ideas SET status = 'Approved'` over every selected row.
  await api('POST', '/api/settings', {
    token: AADMIN,
    body: { approval_stages: 'originator,team_lead,plant_head' },
  });
...
test('changing the chain affects that tenant only, and every user in it', async () => {
  // The chain lives in the tenant's own org_settings, inside the tenant's own database, and
  // getApprovalConfig() reads it per request with no cache.
...
test('an organisation can rename a stage without changing what it means', async () => {
  // Not every organisation has a "Team Lead".
...
test('removing a stage an idea is sitting at does not skip the rest of the chain', async () => {
  // An administrator can edit the chain while ideas are in flight.
...
test('a stage nobody holds is skipped, recorded, and does not hide the idea', async () => {
  ...
  // senior_manager is chosen because nothing earlier in this suite creates one - asserted
  // below rather than assumed, since a test whose premise quietly stops holding passes for
  // the wrong reason.
...
test('a reviewer cannot be the approver of their own idea, even alone in the role', async () => {
  // A team lead who submits an idea is not a team lead who can approve it, so a stage whose
  // only holder is the author is as empty as one with nobody in it - and must be skipped
  // rather than leaving the idea unactionable.
  const aTok = (await login('admin@orga.test', PASSWORDS.orgaAdmin, 'orga')).token;
  await api('POST', '/api/settings', {
...
test('tickets can be archived by status, and only the named statuses move', async () => {
  ...
  // Counted first, because the sweep is deliberately global: it acts on every matching
  // ticket in the registry, not only on the ones this test raised.
  ...
  // Restoring works the same way round By id rather than by status, so this test leaves the
  // registry as it found it: a status sweep here would also un-archive whatever earlier
  // tests filed.
  res = await api('POST', '/api/platform/tickets/bulk-archive', {
...
// The Lifetime plan cannot be removed, and cannot be edited into something that expires.
test('the Lifetime plan is permanent, and stays a lifetime plan', async () => {
  ...
  // Moving it onto a cycle with a length: refused, because that would hand every
  // organisation on it an expiry date.
  res = await api('PATCH', `/api/platform/plans/${lifetime.id}`, {
...
test('an org admin can see the review queue and cannot act on it', async () => {
  ...
  // Needed so the team_lead stage has a holder: an empty stage is stepped over, and the idea
  // would then be waiting somewhere other than where this test says.
  ...
  // Reading: allowed, and the idea really is in there - otherwise "cannot act" would be
  // trivially true because there was nothing to act on.
...
test('an idea reaches the submitter\'s own manager, and no other manager', async () => {
  ...
  // And the screen is not the rule.
...
test('an organisation may have only one plant head', async () => {
  ...
  // Editing the sitting plant head must still work. `assertRoleVacant` excepts the row being
  // edited, or changing their phone number would fail on the grounds that a plant head
  // exists - which they are.
...
test('an idea waiting for a plant head reaches them once one is appointed', async () => {
  ...
  // It waits AT the empty stage, not at the one that just approved it.
  let st = await stageOf(ideaId);
...
// A pooled connection the database has already closed costs one retry, not one error page.
...
test('the dashboard pending count agrees with the review queue', async () => {
  ...
  // The manager submits one of their own.
  ...
  // Asserted by identity rather than by total.
...
test('queued mail is actually attempted, and only silenced by an explicit opt-out', async () => {
  ...
  // The suite has no mail provider, so a send cannot succeed - and that is fine, because the
  // bug was never about delivery.
  await setEnabled('0');
  ...
  // Now with a route. The provider gate below the opt-out is a separate and legitimate check
  // - a deployment with nowhere to send has nothing to try - so the suite gives this tenant
  // an SMTP host that will refuse the connection.
  await setEnabled('1');
  await sql('ifqm_test_a',
...
test('the email queue accepts every status the sender writes, strictly', async () => {
  // Asked through the pool the APPLICATION uses, not through the suite's own connection.
  const { getTenantPool } = await import('../src/database/tenant.js');
  const pool = getTenantPool({ db_name: 'ifqm_test_a', db_host: config.masterDb.host });
...
test('a reward period resolves to a printable, calendar-aligned range', async () => {
  ...
  // Offsets step backwards without overlapping.
...
test('the rewards leaderboard scores the period and lists everyone', async () => {
  ...
  // Ordered on the PERIOD score.
  assert.ok(bRow.rank < qRow.rank,
    'two ideas this period must outrank one this period');

  // The working is shown, and it adds up. A score somebody is rewarded against has to be
...
test('the rewards pack carries every idea, its chain and its timeline', async () => {
  ...
  // The position each approver held AT THE TIME, recorded rather than read off their user
  // row today.
  ...
  // The configured path, so a chain that stopped early is distinguishable from one that ran
  // to completion.
...
test('an ordinary employee cannot pull the rewards pack', async () => {
  ...
  // The org admin CAN. Running a reward cycle is administration, not adjudication: reading
  // who did well is not deciding whether an idea is any good, so this does not cross the
  // line that keeps admins out of approvals.
  const aTok = (await login('admin@orga.test', PASSWORDS.orgaAdmin, 'orga')).token;
...
// A new IFQM staff account proves both channels before the console will do anything for
// it.
test('a new platform admin cannot use the console until both channels are proved', async () => {
  ...
  // A number is required. The account is verified on two channels, and two means two: an
  // address alone can be taken by whoever holds that mailbox.
  ...
  // The code row is seeded rather than sent.
  const bcrypt = (await import('bcryptjs')).default;
  ...
  // And the gate opens on the SAME token.
...
test('platform verification sends only to the address on the account', async () => {
  ...
  // An attacker-supplied destination is simply ignored - the body carries only a channel,
  // and anything else in it has no route into the query.
  ...
  // Asserted as an absence, which holds whether or not the send itself could complete - the
  // suite has no mail provider, and the property under test is about routing, not delivery.
  const rows = await sql('ifqm_test_master',
...
test('the platform admin SMS purpose resolves to a registered DLT template', async () => {
  ...
  // The pair has to match: the id and the text are approved together, so taking one without
  // the other is exactly the mismatch the carrier drops.
  assert.match(r.text, /complete your registration/,
    'the body sent must be the fallback template\'s own approved wording');
});

// Every platform admin is told, every time a company applies - and a notice that fails is
// retried rather than lost.
test('an unsent registration notice is retried, and a sent one is not resent', async () => {
  const svc = await import('../src/services/registrationService.js');

  // Seeded directly. Submitting one end to end needs a consumed email code AND a consumed
  // phone code, which needs a mail provider and an SMS gateway the suite does not have - and
  ...
  // Whether it SENT depends on a mail provider the suite does not have, so the assertion is
  // on the selection, which is the part that can be wrong in a way nobody would notice.
  await sql('ifqm_test_master',
...
test('changing your mobile number requires a code, and tells the old one', async () => {
  ...
  // Confirming with no code, or a wrong one, must change nothing.
  ...
  // The code row is seeded rather than sent: this suite has no SMS gateway, and delivery is
...
test('a platform admin moves their own number by code, not by editing a field', async () => {
  ...
  // Verified up front, so this test is about CHANGING a number rather than about the
  // first-run verification gate, which has its own test.
  await sql('ifqm_test_master',
  ...
  // The assertion the whole flow rests on.
...
test('a mistyped number on a new admin can be corrected, and must be re-proved', async () => {
  ...
  // The account itself is shut out of everything but verification, which is why it cannot
  // correct its own number.
  const { token } = await api('POST', '/api/auth/login', {
...
test('the sign-up form\'s own payload is enough to submit an application', async () => {
  ...
  const application = {
    ...
    // A real GSTIN, check digit and all - computed with the project's own gstinCheckDigit
  ...
  // Still validated when it IS supplied.
