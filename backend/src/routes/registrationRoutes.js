/*
 * The only unauthenticated write path in the API, so it is rate limited on the same
 * footing as login: an anonymous caller can queue applications for a human to read, and
 * nothing else.
 */
...
// The mobile leg. Rate limited on the same footing as the email one - each request costs a
// real message from a gateway that bills per send.
