/** User routes - /api/users/* Ported from the user-management actions of PHP api/users.php. */
...
// Spreadsheets are small even at 20k rows (a few MB). The cap is deliberately tight:
// exceljs decompresses the upload, so an unbounded file is a memory DoS.
...
// Only an org admin (or super admin) may create accounts in bulk - the same guard
// single-user creation uses.
const ADMIN = requireRole('admin', 'super_admin');

// Bulk import declared before the /:id routes so "import" is never mistaken for a user id.
...
// Changing your own number is verified by code - each request costs a real SMS, so both
// legs sit behind the same limiter as the other code endpoints.
