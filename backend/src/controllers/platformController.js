/** Platform controller - HTTP layer over platformService. */
...
// The /tenants/:id/hierarchy endpoint is gone on purpose - it served the tenant's full org
// chart (names, managers, per-person idea counts) to the vendor. tenantDetail now returns
// the aggregate shell instead, and that is the only per-tenant view.
...
// The number is where a sign-in code and a password reset go, so it is a credential in its
// own right - hence a code to the new handset before it is written, and a notice to the
// old one afterwards.
...
// Correcting somebody else's, for the account whose number was typed wrongly when it was
// created and which therefore cannot verify itself out of the hole.
