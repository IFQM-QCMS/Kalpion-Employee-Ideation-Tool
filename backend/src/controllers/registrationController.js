...
export const checkEmail = asyncHandler(async (req, res) => {
  ...
  return respond(res, {
    ...
    // The form uses this to soften its wording: a personal address is a "we can enable this
    // for you" case, not the same kind of no as a malformed one.
...
// The mobile leg of the same exercise. Both must be proved before an application is
// accepted - see submitRegistration.
...
export const approve = asyncHandler(async (req, res) =>
  respond(res, await registrations.approveRegistration(req.params.id, {
    // req.user.id for a platform admin is the string "pa_<n>" (see authService); the registry
    // column stores the numeric platform_admins.id.
    adminId: Number(String(req.user?.id || '').replace(/^pa_/, '')) || null,
    adminName: req.user?.name || null,
    slug: req.body?.slug || '',
    // Chosen by the approver, who has the company's size and turnover in front of them at
    // exactly this moment.
...
// Every other controller in this directory ends with a default export listing its
// handlers, and both routers here import this one with `import * as`, so nothing was
// broken - but a module that is the single exception to a twenty-two-file convention is a
// module somebody will eventually copy as a template.
export default {
